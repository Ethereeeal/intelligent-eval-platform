...
平台提供智能问答交互界面，用户可通过自然语言对话完成文档上传、评测集查看与编辑、知识问答等全部平台功能调用。评测集生成支持从文档生成问答对评测集。
...
| 知识图谱/社区摘要 | 全局主题问题和复杂关系推理 | 增强 |
| 多模态图表理解 | 图片、复杂财务图表和扫描件语义 | 增强 |
...
### 8.20 新增需求与原文档的衔接与影响（更新）

下表在 8.18 基础上补充智能问答交互（8.19）对原文档的影响：
...
| 第 7 节功能范围总览 | 新增一行：智能问答交互 |
| 第 11 节核心数据对象 | 新增 `ChatSession`（对话会话）、`ChatMessage`（对话消息） |
| 第 15 节分阶段实施 | 智能问答交互纳入阶段 2 |
| 第 18 节待确认事项 | 新增：智能问答交互是否需要支持语音输入 |
