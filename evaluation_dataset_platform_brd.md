...
> 文档版本：V1.3（新增评测集三类来源统一管理、EIU 向量化与上下文补全、跨页证据溯源、量化质量评估体系）  
> 编写日期：2026-08-18  
...
7. 对 EIU、QA 候选与上传评测集执行量化质量评估并在平台界面展示，低于阈值时提示用户明确确认（S0 硬门禁不可绕过）；
...
### 5.9 量化质量评估为参考评分，S0 为硬门禁

EIU、QA 与上传评测集三套量化质量评估作为参考评分体系，用于平台界面展示和质量报告；质量评分低于阈值时警告并要求用户明确确认，确认后可继续（提示性门禁）。S0 强制规则（内容安全、隐私、证据核验等）为不可绕过的硬门禁，质量评分高不能豁免。
...
flowchart LR
    ...
    B --> C["语义增强与 EIU 抽取（上下文补全、P0 判定、证据列表化）"]
...
9. **P0 清单视图与用户确认**：抽取完成后展示本批次 P0 清单（完整陈述、P0 规则依据、抽取置信度），默认批量通过，用户只处理异常项（误判 P0、漏抽），标记后走 `revise` 或人工裁决；规则变更走版本化流程，不静默修改。用户确认不能豁免 P0 覆盖率 100% 门禁（BR-005）和 S0 强制规则；P0 降级必须走规则变更流程、留痕、全量重算覆盖率。
...
| FR-SEM-005 双通道校验 | 新增上下文自足性检查、P0 清单视图与用户确认、EIU 质量评估引用 |
...
| BR-018 | 用户确认不得豁免 S0 强制规则与 P0 覆盖率 100% 门禁；P0 降级必须走规则变更流程、留痕并全量重算覆盖率 |
...
| EvaluableUnit | 可评测信息单元（EIU） | 绑定有序证据列表（EvidenceItem）、覆盖状态与 P0 规则依据 |
...
| AC-35 | 平台展示 P0 清单视图并支持批量确认；P0 异常动作需人工确认，且不得豁免 P0 覆盖率 100% 与 S0 强制规则 |
