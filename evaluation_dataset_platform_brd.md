...
> 文档版本：V1.4（新增 EIU 自动质量门禁、文档质量反馈与按用途选题策略）
> 编写日期：2026-09-06
> 项目目录：`micro_innovation`  
> 建议产品代号：EvalForge（评测集锻造平台）  
> 关联文件：生产化前代办与技术债收口见 `docs/production-readiness-todo.md`（历史讨论产物已归档删除，价值并入本 BRD V1.4）

## 1. 文档摘要

本平台面向授信政策、企业财报等专业文档场景，将用户上传的一组文档转化为一套可验证、可回溯、可复现的问答评测集，并自动运行评测、定位失败原因和给出优化建议。平台统一管理三类评测集来源：由文档生成的评测集、用户直接上传的评测集、组织方预置的公共评测集库；Agent 评测前可指定单个评测集、勾选公共库维度或合并多个来源形成临时标准化评测集。

平台的核心不是“让大模型按段落多出题”，而是建立以下闭环：

1. 保存不可篡改的原始文档和版面定位信息；
2. 抽取目录、段落、表格、术语、指标、规则、例外和可评测信息单元（含上下文补全与有序证据列表化）；
3. 同时建立关键词、向量和语义关系索引，其中 EIU 是向量化与跨文档知识融合的核心对象（chunk 是文档切分的中间结构，主要服务检索）；
4. 按单段、跨段、跨文档、改写、安全边界等类型生成问题及标准答案；
5. 用"可评测信息单元覆盖率"而不是"问题数量"计算覆盖率；EIU 覆盖率、85% 门禁与 P0 全覆盖仅适用于文档生成评测集；
6. 对每个答案保存可回溯的原文证据和推理路径；
7. 对 EIU、QA 候选与上传评测集执行分层质量控制：EIU 自动质量门禁决定是否进入出题，QA 质量检查决定样本能否发布；S0 硬门禁不可绕过；
8. 自动执行目标问答系统，分别评估检索和答案生成；
9. 将失败案例归因后，优化文档解析、语义表示、检索策略或评测题本身，再进行回归测试。
...
9. **评测集来源分散、缺少统一管理。** 文档生成、历史 QA 数据、公共评测集各成体系，质量口径和维度归类不一致，无法统一评估与组合使用。
10. **缺少量化质量评估。** EIU、QA 与上传评测集是否合格缺少可量化指标，质量依赖人工抽查，无法在平台界面展示或设置提示性门禁。

## 3. 产品定位

### 3.1 产品定义

EvalForge 是一个"评测数据资产管理 + 自动构建 + Agent 能力评估 + 智能问答交互"平台。它以 EIU 为连接知识库、评测集生成与评测质量控制的中间层，统一管理文档生成评测集、直接上传评测集与公共评测集库三类来源，产出标准化、可导出的评测数据，并通过适配器调用待测系统。它不绑定某一种 RAG 框架或大模型。
...
| BG-09 | 统一管理三类评测集来源 | 文档生成/直接上传/公共评测集库均纳入统一管理与质量评估 |
| BG-10 | Agent 评测前按需组合评测集 | 支持指定单个评测集、勾选公共库维度、合并多来源形成临时标准化评测集 |
| BG-11 | 量化质量评估可见可用 | EIU/QA/上传评测集三套质量评估指标在平台界面展示，低分样本经用户确认后可继续 |
...
- 对最终版本执行确认、发布和导出；
- 配置 Agent 评测前的评测集组合选择（指定单个评测集、勾选公共库维度或合并多个来源）；
- 对质量评分低于阈值的样本、P0 异常动作（删除/停用 P0 样本、P0 未覆盖仍想发布、把 P0 移出分母）执行明确确认并说明理由；
- 一次性确认 P0/P1/P2 规则包与质量评估阈值等预置配置（不逐条确认 P0）。
...
| 向量存储/索引 | 保存 EIU、文段、上下文说明和语义摘要的向量及其业务主键映射（EIU 为向量化核心对象，见 5.7） |
...
| 公共评测集库 | 组织方（银行/平台运营）预置内置的标准评测集，按可配置维度分类，版本化管理；用户只可查看和选择使用，不开放共享入库 |
| 评测集维度体系 | 测试维度做成可配置体系，待首批公共 QA 数据集确定后固化，不在 BRD 写死示例维度 |
| 质量评估配置 | EIU/QA/上传评测集三套量化质量评估的指标、阈值与评分策略（含语义评分固定校准集）为后台配置并版本化 |
...
### 5.7 EIU 是向量化与跨文档知识融合的核心对象

chunk 是文档切分的中间结构，主要服务检索；EIU（可评测信息单元）承载完整语义，是向量化存储与跨文档、跨 chunk 问题生成及知识融合的基础。跨文档/跨 chunk 的问题生成应基于 EIU 相似度聚类、融合，而不是以 chunk 为向量化对象直接生成。

### 5.8 三类评测集来源统一管理

平台统一管理文档生成评测集、直接上传评测集与公共评测集库三类来源。EIU 覆盖率、85% 门禁与 P0 全覆盖仅适用于文档生成评测集；直接上传评测集与公共评测集库只包含 QA 对、不产生 EIU，只做质量评估与组合选择，不参与 EIU 覆盖率计算。

### 5.9 EIU 自动质量门禁、文档反馈与 S0 硬门禁

EIU 是文档进入自动出题流水线的中间产物，不应形成要求用户逐条确认的大规模审核队列。系统必须在后端自动完成知识点的忠实性与可追溯性、上下文完整性、原子性检查，以及文档相对重要性判断；只有通过自动门禁的绿色知识点可进入 QA 生成。

文档质量评估是与 EIU 抽取同步产生的自动诊断报告，用于反馈原文的定义缺失、引用无法解析、上下文不足、表格结构缺失和规则冲突等问题；它不使用单一总分、不阻塞用户操作，也不要求用户逐项确认。S0 强制规则（内容安全、隐私、证据核验等）仍为不可绕过的硬门禁，任何质量状态或人工操作均不能豁免。
...
flowchart LR
    ...
    B --> C["语义增强与 EIU 抽取（同文档补证、归并、重要性判断、证据列表化）"]
    C --> D["多路索引：EIU 向量化 + 文段向量/关键词/关系"]
    D --> E["单段问题生成"]
    D --> F["跨段/跨文档候选发现（EIU 相似度聚类融合）"]
    F --> G["多跳问题生成"]
    E --> H["改写、反例与安全问题生成"]
    G --> H
    H --> I["答案、证据与推理路径校验（继承 EIU 证据列表）"]
    I --> J{"覆盖率和质量门禁通过？"}
    J -- "否" --> K["补题、修题或审核 Skill 驳回"]
    K --> I
    J -- "是" --> L["冻结并发布评测集版本"]
    U["上传已有 QA 评测集"] --> V["格式校验 + 质量评估 + 治理审核"]
    V --> L
    W["公共评测集库（组织方预置、维度可配置）"] --> X["Agent 评测前组合选择"]
    L --> X
    X --> M["调用待测系统自动运行"]
    M --> N["检索、生成、安全分层评分"]
    N --> O["失败归因与优化建议"]
    O --> P["智能体维护方更新版本后，用同一冻结评测集复测"]
    P --> M
```
...
| 语义理解 | 摘要、术语、实体、指标、可评测信息单元（含上下文补全与证据列表化）、关系 | 核心 |
| 多路索引 | EIU 向量化 + 关键词、文段向量、元数据过滤、父子结构 | 核心 |
| 单段问答生成 | 问题、答案、证据、质量校验 | 核心 |
| 跨段/跨文档生成 | 候选发现、关系确认、多跳问题 | 核心 |
| 改写与反例 | 多表达变体、易混概念、不可回答题 | 核心 |
| 覆盖率与审核 | 覆盖计算（仅文档生成评测集）、缺口清单、审核 Skill、发布阻断 | 核心 |
...
| 评测集回流 | 不纳入平台；智能体失败由自动评测诊断与优化处理，评测集内容问题走质量复核与版本编辑 | 架构决策 |
| 空评测集处理 | 无实质内容语料库判定、空集发布与门禁衔接 | 核心 |
| 评测集管理（三类来源统一） | 文档生成/直接上传/公共评测集库统一入库、版本与质量评估 | 核心 |
| 评测集组合选择 | Agent 评测前指定单个评测集、勾选公共库维度、多来源合并成临时标准化评测集 | 核心 |
| 量化质量评估展示 | EIU/QA/上传评测集三套质量评估指标在平台界面展示，低分样本提示确认 | 核心 |
...
背景信息（章节主题、文档背景、行业常识等）应放入上下文说明层，不参与 EIU 的独立性判定；EIU 的"独立"指可独立判定真假（仅凭 EIU 陈述 + 引用证据可验证），不是"脱离上下文可读"，也不是"位于同一自然段"。
...
**独立性定义**：EIU 的"独立"指**可独立判定真假**（仅凭 EIU 陈述 + 引用证据可验证），不是"脱离上下文可读"，也不是"位于同一自然段"。高关联、跨页、跨自然段的知识点不是合并或拆碎的理由，也不是独立性判断失败的理由；"引用未展开"才是失败原因。

**上下文补全/物化**：真值条件（主体、条件、范围、期间、币种、单位、例外）应补全进 EIU 陈述，宁可陈述长一点；补全不新增 EIU 数量、不重复计覆盖率。三类上下文的处理方式：
...
4. EIU 证据范围可跨块（见下方"证据列表化"）；
5. 需要组合多个 EIU 才能回答的题走跨段/跨文档多跳路径（8.7），不在抽取阶段预合并。

**证据列表化（存储/审计层）**：EIU 的证据是**有序证据列表**，不是"一块原文"：

- 每条证据引用原文片段（不复述 EIU 陈述，防止循环溯源），并复用 FR-PARSE-003 全字段定位（`document_id`、`document_version`、`section_path`、`page_no`、`block_id`、原文起止偏移/坐标、原文片段、解析器版本）；
- 每条证据带角色编码：`direct`（正文依据）、`qualifier`（条件来源）、`reference`（原文引用关系）、`conflict`（冲突/反证）；
- `statement_provenance`（可选增强）：把 EIU 陈述按片段拆开，映射到对应证据条目（如"适用对象→证据 0、结论→证据 1"），供机器校验条件与结论的来源；
- 每条证据带 `document_version` 和 `parser_version`；EIU 带 `evidence_list_version`；
- 审查视图层只展示"原文引用片段 + 位置标签（页码·条款）+ 角色中文名"，全字段折叠在"详情"中；两层共享同一份数据，审查视图只是投影，不额外维护简化证据（见 8.13 展示视图）。

**补全必可溯源（硬规则）**：上下文补全不得无证据。陈述中每个补进来的限定语，必须在证据列表中找到对应块；找不到即视为模型臆造，判 `revise` 或 `block`。

**P0 判定机制**：内容优先级不以 LLM 自由语义判断为准，采用四层机制：

1. **确定性规则为主**：按 EIU 类型 + 预置规则包映射（监管/禁止/阈值/例外/安全边界 → P0），规则编号进入 EIU 记录；
...
- 主体、动作/关系、客体或结论；
- 条件、例外、范围、期间、单位等限定信息；
- EIU 类型、相对文档目的的重要性、重要性理由和权重（重要性不由固定 P0 规则依据直接决定）；
- 原文证据范围与有序证据列表（含角色与版本）；
- 是否可出题及排除原因；
...
9. **拆与合判定标准**：两条陈述可分别被证明或否定 → 拆；一条限定语同时作用于多条规则 → 每条 EIU 各自补全该限定语（或引用），不另计；必须合在一起才能判定真假的才合（如"A 和 B 共同构成 X 的完整定义"）。既有规则不变：例外单列、定义与公式分列、表格业务行、重复合并、不可回答排除。
...
8. **上下文自足性检查**：对高关联文档增加"上下文自足性"检查，防两类失真：
   - 过度合并：一串关联内容包成一条大而空、无法验证的 EIU；
   - 过度拆分：上下文剥离后产生歧义或错误归属的 EIU。
   不满足的判 `revise`，并注明是"缺限定语"还是"需要补关系"。
9. **重要性审计与状态展示**：抽取完成后保存规范知识点簇的相对文档目的重要性、理由、策略版本与证据。用户界面只展示“正常知识点 / 待复核 / 已排除”，不得展示 P0 清单或要求逐条确认；重要性策略变更走版本化流程并全量重算。任何人工指定纳入仍不得豁免证据门禁、测试全量测的绿色 P0 覆盖要求（BR-005）或 S0 强制规则。
10. **EIU 质量评估**：EIU 清单形成后执行 EIU 质量评估（四项必查：完整性、独立性、重复率、可生成性；其余软指标只作参考展示），具体见 FR-QA-005。
...
3. **向量存储/索引**：保存 EIU 及原文文段、上下文说明、摘要的 Embedding 向量及业务主键映射；EIU 是向量化与跨文档知识融合的核心对象，chunk 是文档切分的中间结构、主要服务检索；
...
2. 稠密向量索引：召回语义相近但表达不同的内容；以 EIU 向量为主、文段/上下文/摘要向量为辅；
3. 元数据索引：按语料库、文档、章节、主体、期间、指标等过滤；
4. 结构/关系索引：沿父子节点、引用、规则—例外、指标关系扩展证据；
5. 表格索引：保留表头、行列关系、单位和单元格坐标。

跨文档、跨 chunk 的问题生成基于 EIU 相似度聚类、融合，而不是以 chunk 为向量化对象直接生成（见 5.7）。
...
**适用范围**：EIU 覆盖率、85% 门禁与 P0 全覆盖仅适用于**文档生成评测集**；直接上传评测集与公共评测集库不产生 EIU、不参与覆盖率计算（见 8.22）。

**覆盖补强机制**：

- 量化指标**实质文段 EIU 关联率**：已拆出 EIU 的实质文段比例（不只查"有没有关联"）；
- P0/P1 所在文段**逐段核对 EIU 是否拆全**；
- **抽样人工复核**：随机抽段落核对漏抽情况，估算抽取召回率（类似检索 Recall 思路）并展示；
- **分母排除率异常阈值**：排除比例异常偏高时强制人工确认（结合 FR-COVER-003）。
...
分母排除率应作为量化指标监控：排除比例异常偏高时强制人工确认，并披露排除原因分布。P0 EIU 移出分母必须走人工确认流程并说明理由，且不得豁免 P0 覆盖率 100% 门禁（BR-005）；P0 降级必须走规则变更流程、留痕、全量重算覆盖率。
...
`EvalCase.evidence` 继承 EIU 的完整证据列表（不是只带结论块）；每个答案要点绑定对应证据，验证时同时满足所有限定前提；失败归因时证据链每一跳可定位到原文（机器层），审查时以可读视图展示（FR-SEM-003 证据列表化）。
...
1. 对每个新增 EIU 仅检索同一语料库内 Top-K 近邻（基于 EIU 向量相似度聚类），并结合文段级检索补充候选；
...
#### FR-QA-005 质量评估参考评分体系

平台对 EIU、QA 候选与上传评测集执行三套量化质量评估，作为**参考评分体系**，用于平台界面展示和质量报告：

- **EIU 质量评估**（四项必查 + 参考指标）：
  - 必查项：完整性（真值条件：主体/条件/范围/期间/单位/例外是否齐全）、独立性（是否依赖未声明的上下文才能判定真假）、重复率（分母去重，防止覆盖率虚高）、可生成性（是否适合出题，决定是否进覆盖率分母）；
  - 参考指标（只展示不设门禁）：信息密度、覆盖范围评分等；
  - EIU 质量评估**不能整体跳过**：QA 评估检查"产品合格"，EIU 评估检查"原料合格"，QA 通过率无法替代（EIU 拆分错误、真值条件缺失、分母脏时，QA 仍可能 100% 通过）。
- **QA 质量评估**：问题清晰度、答案完整性、问答一致性、难度等级、重复率；
- **上传评测集质量评估**：数据完整率、重复问题比例、有效 QA 比例、覆盖维度（用于直接上传评测集与公共评测集库，见 8.22）。

**数据集质量报告分层展示**：

1. 文档→EIU 层：实质文段对账率、实质文段 EIU 关联率、EIU 完整性通过率；
2. EIU→QA 层：加权 EIU 覆盖率；
3. QA 层：QA 通过率、重复率。

"QA 通过率"是流水线良率指标，不是评测集质量的完整定义。

**门禁关系**：

- 质量评分低于阈值时，系统警告并要求用户明确确认，确认后可继续（提示性门禁）；
- S0 强制规则（GOV-CONTENT-001 等）为不可绕过的硬门禁，质量评分高不能豁免；
...
平台交付一份完整、冻结、可重复使用的评测集，不区分开发集、验证集和锁定测试集；平台不负责对待测智能体调优。每个评测运行均基于指定的冻结评测集版本，记录待测智能体的适配器、模型/配置版本和运行时间，以便比较同一评测集上不同智能体版本的分数及各维度变化。

评测集质量问题只能在进入 m08 自动评测前，由人工走 m04 质量复核、m05 候选题修订和显式冻结新版本处理；一旦开始 m08 运行，不得因该运行的结果修改其评测集或覆盖历史结果。
...
- P0 异常动作（删除/停用 P0 样本、P0 未覆盖仍想发布、把 P0 移出覆盖率分母）必须人工确认并说明理由，且不得豁免 P0 覆盖率 100% 门禁与 S0 强制规则（BR-005、BR-018）。
...
- 评测集内容问题仅在进入 m08 前由人工发起编辑草案；m08 运行结果不得自动或一键转入评测集编辑；
...
证据展示采用分层视图（与 FR-SEM-003 证据列表化共用同一份数据）：默认只显示原文引用片段 + 位置标签（页码·条款）+ 角色中文名；跨页时展示角色与关系连线（如"条件来源 第1页 → 正文依据 第3页"）；块编号、偏移量、解析器版本、证据列表版本等全字段审计信息折叠在"详情"中。
...
- 文档更新后按证据条目对比变化（哪条失效、哪个限定语被替代），支持精确增量定位；历史版本证据引用不被改写；
...
### 8.16 评测结果使用（架构决策：不做评测集回流）

自动评测的 ErrorBook、失败归因和优化建议仅用于待测智能体的评分、诊断、优化与回归比较，不反向驱动评测集修改。智能体回答不佳默认属于待测系统问题，不能据此降低评测集质量结论。人工发现题目、答案或证据本身存在缺陷时，走 FR-DS-EDIT 的质量复核、草案编辑和新版本流程；不建设 `evaluation_feedback`、回流工作台或 FR-DS-FB 闭环。
...
| 12.3 审计 | 扩展编辑类、替换类与评测运行操作留痕（FR-DS-EDIT-004、FR-DS-GEN-001、FR-DIAG） |
...
| FR-DIAG-001 / FR-DIAG-002 / FR-OPT-001 / FR-OPT-003 | 黑盒失败记录、跨版本复测与 ErrorBook 聚类；不建设评测集回流闭环 |
...
评测集管理模块统一管理三类评测集来源，而不是简单"上传评测集/公共库"：

1. **文档生成评测集**：上传文档 → 解析 → 分块 → 向量化存储 EIU → EIU 质量评估 → QA 生成 → QA 质量评估 → 形成评测集（主线 8.1–8.10）；
2. **直接上传评测集**：上传已有 QA 数据 → 格式校验 → 质量评估 → 治理审核 → 入库；
3. **公共评测集库**：组织方预置内置的标准评测集，按可配置维度分类，长期维护、版本化管理。

EIU 覆盖率、85% 门禁与 P0 全覆盖仅适用于**文档生成评测集**；直接上传评测集与公共评测集库只包含 QA 对、无源文档、不产生 EIU，不参与 EIU 覆盖率计算，只做质量评估与组合选择。

#### FR-DS-SRC-001 直接上传评测集（单轮模板）

平台应支持用户上传已有 QA 评测集，单轮样本模板字段：
...
| `dimension` | 否 | 测试维度归类（维度体系待定，见 FR-DS-SRC-004） |

模板明确不含：同义答案（担心歧义）、数值容差、预期行为、intent_id、corpus_id、内容优先级、数据集划分。同义答案由语义评分处理（见 FR-DS-SRC-003），不进模板字段。支持格式与平台导出格式一致（JSON/JSONL/Excel/CSV），MVP 阶段由平台侧配置并校验。

#### FR-DS-SRC-002 多轮评测集模板（可选模式）

用户可选择是否启用多轮测试，模板为 `session_id` + `turns[]`：

- 最终轮 `a` 必填，作为最终标准答案；至少要有最终轮标准答案，无关键轮时报告标注"仅覆盖最终结果"；
...
- 运行记录必须保存完整对话过程，用于复现该次黑盒评测，不据此推断内部根因（FR-DIAG-001）。

#### FR-DS-SRC-003 评分口径（运行侧配置）

- 短答案（数值、日期、条款号、固定短语、拒答）→ 规范化精确匹配（去空白、全半角统一、数字格式标准化）；
- 自然语言长答案 → 语义相似度评分（embedding / 评分模型 + 阈值），阈值须经固定校准集验证；采用 LLM 裁判时须防位置偏差与长度偏差，并保留人工抽查；
- 评分策略为平台运行侧配置，不进模板字段；评分模型、阈值与版本必须记录，保证可复现、可解释。

#### FR-DS-SRC-004 公共评测集库
...
- **维度分类**：维度体系暂不固定，待首批公共 QA 数据集确定后再定；系统维度做成**可配置体系**，BRD 不写死示例维度（合规安全/性能稳定/准确性/场景可用性仅为示例）；
- 公共评测集库条目进入评测集组合选择（FR-DS-SRC-005）。

#### FR-DS-SRC-005 Agent 评测前的评测集组合选择

Agent 评测前（第 9 节自动运行），用户应能：

- 直接选择某个已有评测集（指定单个评测集）；
- 勾选公共评测集库中的某些维度；
- 组合多个来源（文档生成评测集、上传评测集、公共库维度）形成**临时标准化评测集**。

组合选择结果应作为评测运行配置记录、版本化并参与审计；评测指标按组合后的评测集口径统计。

#### FR-DS-SRC-006 无证据样本降级标注

- 上传评测集与公共库样本如未填写 `evidence`，标记为"无证据样本"；
- 无证据样本不参与证据回溯率统计（证据回溯率仅适用文档生成评测集）；
- 展示与导出时对无证据样本带降级标注，不能伪装成有证据样本。

### 8.23 增量需求与原文档的衔接与影响（再次更新）

下表汇总 V1.3 新增需求（评测集三类来源统一管理、EIU 向量化与上下文补全、跨页证据溯源、量化质量评估、P0 机制）对原文档各条目的衔接与影响：

| 原文档条目 | 本版影响 |
|---|---|
| 3.1/3.2 产品定位与目标 | 定位升级为"评测数据资产管理 + 自动构建 + Agent 能力评估"，新增 BG-09/10/11 |
| FR-SEM-001 文段上下文化 | 明确背景信息外置，不参与 EIU 独立性判定 |
| FR-SEM-003 EIU 抽取 | 独立性定义、上下文补全/物化、证据列表化、P0 判定机制、记录字段扩展 |
| FR-SEM-004 拆分与计数 | 新增拆与合判定标准 |
| FR-SEM-005 双通道校验 | 新增上下文自足性检查、重要性审计、三项 EIU 门禁与文档质量反馈引用 |
| FR-STORAGE-001 / FR-INDEX-001 / FR-MH-001 | 向量化对象从 chunk 改为 EIU，EIU 相似度聚类作为跨文档融合基础 |
| FR-COVER-002/003 覆盖率 | 明确仅适用文档生成评测集；新增实质文段 EIU 关联率、逐段核对、抽样复核、分母排除率监控 |
| FR-QG-003 证据绑定 | EvalCase 继承 EIU 完整证据列表 |
| FR-QA-001/002/003 | 新增 FR-QA-005 质量评估参考评分体系；S0 硬门禁不变 |
| FR-DS-EDIT-002 删除与停用 | P0 异常动作（删除/停用、未覆盖发布、移出分母）必须人工确认，不得豁免门禁 |
| FR-DS-INC-001 增量回写 | 按证据条目对比变化，历史证据引用不被改写 |
| 8.13 目录浏览与导出 | 新增证据展示分层视图（L1 可读证据 / L2 角色关系 / L3 全字段详情） |
| 第 7 节功能范围总览 | 新增三类来源统一管理、评测集组合选择、量化质量评估展示 |
| 第 9 节自动运行 | 适配器支持多轮会话；FR-METRIC-001 新增质量评估指标与三层报告 |
| 第 10 节业务规则 | 新增 BR-017 至 BR-021 |
| 第 11 节核心数据对象 | 新增 EvidenceItem、StatementProvenance、UploadedEvalSet、PublicEvalSet、EvalSetDimension、EvalSetComposition |
| 第 12.3 节审计 | 扩展组合选择、公共库维护、P0 确认、质量评估评分留痕 |
| 第 14 节验收标准 | 新增 AC-29 至 AC-38 |
| 第 15 节分阶段实施 | 阶段 0 确认 P0 规则包与维度体系；Demo 纳入上传评测集、公共库、组合选择与质量评估展示；阶段 2 纳入多轮评测与证据视图深化 |
| 第 18 节待确认事项 | 新增上传格式、公共库首批数据集与维度体系、评分校准集、门禁阈值等确认项 |


### 8.24 EIU 自动质量门禁、文档质量反馈与评测集用途策略（V1.4）

本节为已确认的产品口径。若与 V1.3 中“按 EIU 类型或预置规则包直接判 P0”“低质量 EIU 需用户逐条确认”“前端展示 P0 清单/标签”等表述冲突，以本节为准。

#### FR-EIU-GATE-001 规则识别与文档级重要性分离

规则抽取负责识别候选的 `eiu_type`、主体、结论、条件、例外、数值及证据，不得直接写入最终 P0/P1/P2。`不得`、`禁止`、`必须`、阈值和例外仅是重要性判断信号，不能机械决定优先级。

系统应以文档标题、前言/摘要、目录、章节结构、同文档的规范知识点簇和原文证据为输入，批量判断每个规范知识点簇的**相对文档目的的重要性**：

- **P0（必覆盖）**：遗漏会使文档核心结论、关键边界、风险控制或关键执行要求被明显误解；
- **P1（应覆盖）**：有助于正确理解，但遗漏不会改变文档核心结论；
- **P2（可选）**：背景、示例、重复表述或低价值延伸。

重要性结论必须保存理由和模型/规则版本。P0/P1/P2 是后端策略与覆盖统计字段，**前端不显示 P0/P1/P2，也不将既有“待复核”“已排除”等用户可见状态改名为 P0/P1/P2。**

#### FR-EIU-GATE-002 同文档补证、原子性与归并

上下文补全只允许在同一文档内检索，可使用章节路径、父条款、同条款 Block、相邻 Block、显式引用条款、术语定义和表格表头。每个补进的事实必须记录 `document_id`、`block_id`、`section_path` 和证据角色；不得跨文档借用、推断或补写规则。

一条规范知识点对应一个独立的业务判断意图。自动归并仅适用于同一业务场景、相同结论、相同适用范围和条件下的重复、同义或完整正反表达；主体、条件、例外、阈值、时效或业务结果不同，且可分别考察时，必须保留为不同知识点。归并保留全部原始候选与证据，不物理删除来源记录。

#### FR-EIU-GATE-003 三项知识点硬门禁与自动处置

知识点阶段仅执行以下三项硬门禁：

1. **忠实性与可追溯性**：陈述的结论、条件与例外均有原文证据，且可精确定位；
2. **上下文完整性**：定义、指代、引用、前置条件、例外及表格结构已补足；无法从同文档补足时记录原因；
3. **原子性**：一条知识点只承载一个独立业务判断意图，多个可独立判断的规则已拆分，重复表达已归并。

“可测试性”不作为知识点质量维度，不得再以是否命中谓词、数字或固定句式决定知识点是否通过。实际问题是否明确、答案是否可由证据推出，属于 QA 生成与 QA 质检阶段。

自动处置规则如下：

| 重要性 | 自动处理 | 用户可见结果 | 是否进入出题 |
|---|---|---|---|
| P0 | 同文档补证、扩大上下文、LLM 二次审查 | 未解决时为“待复核”（黄色），可编辑、排除或人工指定纳入 | 否，直至重新校验通过 |
| P1 | 一次补证与重审 | 未解决时为“已排除”（红色、只读） | 否 |
| P2 | 首次质量审查 | 未解决时为“已排除”（红色、只读） | 否 |
| 任意等级 | 三项硬门禁通过 | 正常知识点（绿色） | 是 |

红色记录只做可追溯归档，不出现在默认知识点表格中；用户可从既有“已排除”入口查看原文、证据与自动原因，但不可编辑。黄色记录继续使用既有“待复核”入口与筛选能力，不向用户暴露其 P0 内部标签。

#### FR-EIU-GATE-004 文档质量自动反馈

系统应与 EIU 抽取同步生成文档质量反馈，不设总分，也不创建必须人工确认的待办。反馈至少包括：关键定义缺失、引用目标无法解析、关键规则缺少上下文、表格缺表头/单位、同文档规则冲突、P0 知识点经自动补证后仍无法通过门禁。每项反馈必须包含影响范围、原文 Block 定位和系统已采取的处理。

文档库可展示“正常 / 有质量提示 / 有关键质量风险”摘要；知识点页展示问题摘要与定位入口。该状态只说明原文对自动构建评测集的影响，不替代人工业务审核，也不要求用户确认。

#### FR-EIU-GATE-005 QA 边界与人工指定纳入

绿色知识点表示其原文依据充分、语义边界清晰、允许进入 QA 生成；QA 生成器的超时、格式错误、重复题、答案超出证据或模板不合规，首先按生成层故障重试或记录，不得自动把知识点降级或将其归因为文档质量问题。若同一绿色知识点持续无法产出合格 QA，应作为门禁或生成器的改进信号进行监控。

用户可对黄色或未被当前用途选中的绿色知识点执行“人工指定纳入”，并填写原因。系统必须重新执行三项知识点门禁；仅在证据可追溯时才允许生成或手工编辑 QA。人工指定不改写自动状态或重要性结论。红色“已排除”记录保持只读、不可操作；无原文证据的试验题可以单独保留，但不得进入正式评测集或覆盖率分母。

#### FR-EIU-GATE-006 按用途构建评测集

同一绿色知识点和 QA 库可派生不同用途的评测集，不复制知识点：

| 用途 | 目标 | 选题规则 |
|---|---|---|
| 开发自测 | 快速发现明显功能、检索和理解错误 | 覆盖全部核心规则簇，优先代表性 P0 与关键例外；默认上限 10–20 题，按规则簇覆盖优先而非随机截断 |
| 测试全量测 | 验证完整覆盖与边界 | 纳入全部绿色 P0/P1/P2，以及关键例外、组合条件和边界题 |
| 业务测 | 验证业务人员实际关心的回答效果 | 选择业务价值高的 P0/P1，排除纯技术、背景与低价值延伸题 |

每个 QA 可保存“评测集适用性”（开发自测、测试全量测、业务测）。该字段由系统结合评测目标自动判断，不要求维护银行、技术或大模型领域主题字典；它与 P0/P1/P2 的文档相对重要性独立。

#### FR-EIU-GATE-007 策略版本与历史重算

文档内容更新继续走“重新解析 → EIU 重抽 → QA 重建 → 新版本”闭环。抽取、归并、重要性或门禁策略更新时，即使原文未变化，也必须按质量策略版本重新计算 EIU、失效或重建关联 QA，并保留冻结评测集快照。测试数据允许全量重算；历史自动结论可审计，但默认展示当前策略版本的结果。
...
- 问题（多轮模式下传入完整对话过程 `session_id` + `turns[]` 及关键轮标记 `key_turn`、`depends_on_turns`）；
- 待测智能体版本或配置标识；
- 会话是否清空（多轮适配器支持时）。
...
待测智能体按黑盒接入，只返回最终答案及运行信息；其检索内容即使返回也无法与平台内部 Block/EIU 稳定映射。因此平台不计算检索、排序或上下文归因指标，答错仅记录为端到端失败，不对检索或生成根因作伪精确判断。

多轮评测（8.22 FR-DS-SRC-002）运行时，适配器应支持按 `session_id` 传入完整对话过程，并返回每轮输出与最终答案；运行记录用于复现该次评测，不据此推断内部检索或生成根因。
...
| 实质文段 EIU 关联率 | 已拆出 EIU 的实质文段比例（补充对账率的粒度，见 FR-COVER-002） |
| EIU 完整性通过率 | EIU 真值条件齐全（完整性必查通过）的比例 |
| EIU 独立性通过率 | EIU 不依赖未声明上下文的比例 |
| EIU 重复率 | EIU 之间语义重复的比例（分母去重） |
| 分母排除率 | 被排除出覆盖率分母的 EIU 比例，异常偏高时触发人工确认 |
| QA 通过率 | 流水线良率指标（QA 质量评估通过比例），不是评测集质量的完整定义 |

数据集质量报告按三层展示：文档→EIU（对账率、EIU 关联率、完整性/独立性通过率）→ EIU→QA（加权覆盖率）→ QA（QA 通过率、重复率），见 FR-QA-005。
...
待测智能体不提供可与平台内部 Block/EIU 对齐的检索轨迹，平台仅记录以下可观测结果：

| 标记 | 判定信号 | 说明 |
|---|---|---|
| E2E 端到端失败 | 最终答案未通过评分 | 仅说明本次黑盒回答未达到 Gold；不推断检索、上下文或生成根因 |
| D9 系统运行异常 | 超时、接口错误、限流 | 可观测工程异常，供待测智能体维护方排查 |

#### FR-DIAG-002 归因边界

平台不自动判定 D1–D8，也不以评测结果修改评测集或调优待测智能体。若待测智能体维护方自行具备内部观测能力，应在其自身系统中完成检索、上下文和生成根因分析；平台只提供固定评测集上的结果与跨版本分数比较。

### 9.4 失败记录与跨版本复测

#### FR-OPT-001 失败结果说明

平台为 `E2E` 记录“原因不可定位”的说明，为 `D9` 记录运行异常信息；这些记录仅供待测智能体维护方参考。平台不实施智能体调优，也不根据评测结果修改评测集。

#### FR-OPT-002 稳定评测与跨版本比较

- 同一比较应固定使用同一冻结评测集版本，避免样本变化干扰智能体版本分数；
- 每个待测智能体版本或配置形成独立运行记录，并保留总体分数及类型、难度、维度等分组结果；
- 报告应展示新旧智能体版本的总体分差和各分组变化，便于识别退化；
- 评测平台只负责复测与结果比较，不负责提示词、索引、模型或检索策略调优；
- 评测集如在 m08 前人工修订，必须冻结为新版本；历史运行结果不得被覆盖。
...
| BR-005 | 文档生成的测试全量评测集不得在存在未覆盖的绿色 P0 知识点时发布；P0 仅表示相对文档目的的必覆盖项 |
| BR-006 | 跨文档检索不得越过语料库或权限边界 |
| BR-007 | 冲突文档不得自动无痕合并 |
| BR-008 | 所有题目必须通过评测治理审核 Skill；S0 失败不得发布 |
| BR-009 | 同一 `intent_id` 的所有变体必须处于同一冻结评测集版本 |
| BR-010 | 无答案题必须有明确的预期拒答行为 |
| BR-011 | m08 运行结果不得修改评测集或覆盖历史运行；跨智能体版本比较必须引用冻结评测集版本 |
...
| BR-017 | EIU 覆盖率、85% 门禁与 P0 全覆盖仅适用于文档生成评测集；直接上传评测集与公共评测集库不参与 EIU 覆盖率计算 |
| BR-018 | 用户确认不得豁免 S0 强制规则与测试全量评测集的绿色 P0 覆盖要求；重要性策略变更必须版本化、留痕并全量重算 |
| BR-019 | EIU 上下文补全不得无证据；补进陈述的每个限定语必须有对应证据块，否则判 revise 或 block |
| BR-020 | 无证据样本必须带降级标注，不参与证据回溯率统计 |
| BR-021 | 公共评测集库由组织方预置维护并版本化；用户只可查看和选择使用，不开放共享入库 |
| BR-022 | 知识点补证只可在同一文档内进行；每项补证必须可定位到证据 Block，跨文档不得作为知识点补证来源 |
| BR-023 | P0/P1/P2 是相对文档目的的重要性，不由规则类型、固定领域字典或单条强制词直接决定 |
| BR-024 | 仅通过忠实性与可追溯性、上下文完整性、原子性三项门禁的绿色知识点可进入正式 QA 生成 |
| BR-025 | P0 自动处理后仍未通过的知识点显示为“待复核”；P1/P2 未通过的知识点归档为“已排除”；前端不展示 P0/P1/P2 标签 |
| BR-026 | 文档质量反馈必须由抽取结果自动生成、可定位原文、无单一总分且不要求用户逐项确认 |
| BR-027 | QA 生成层故障不得自动反向改写知识点质量或文档质量；证据不合格的 QA 不得进入正式评测集 |
| BR-028 | 人工指定纳入不得绕过证据可追溯性、S0 门禁和正式覆盖率口径 |
| BR-029 | 开发自测、测试全量测和业务测从同一绿色知识点/QA 库按用途策略派生，不复制知识点 |
...
| EvaluableUnit | 可评测信息单元（EIU） | 绑定有序证据列表、三项门禁结果、相对文档目的的重要性与规范知识点簇 |
| CanonicalEiuIntent | 规范知识点簇 | 归并重复表达并保留多个原始候选和证据，不合并独立可考察点 |
| DocumentQualityFinding | 文档质量自动反馈项 | 指向问题 Block、影响范围、自动处理和策略版本；不要求人工确认 |
...
| EvaluationPurposeProfile | 评测集用途策略 | 开发自测、测试全量测或业务测的选题规则与版本 |
| ManualInclusionOverride | 人工指定纳入记录 | 保存原因、重校验结果与是否计入正式覆盖率 |
...
| FeedbackItem | 不纳入平台数据模型；智能体失败诊断由 ErrorBookItem 表达，评测集内容问题走版本编辑审计 |
...
| EvidenceItem | EIU 有序证据列表中的单条证据 | 含角色、定位全字段、版本信息 |
| StatementProvenance | EIU 陈述片段到证据条目的映射（可选增强） | 关联 EvaluableUnit 与 EvidenceItem |
| UploadedEvalSet | 用户直接上传的 QA 评测集（单轮/多轮模板） | 关联维度配置与质量评估结果 |
| PublicEvalSet | 组织方预置的公共评测集库条目 | 按可配置维度分类，版本化管理 |
| EvalSetDimension | 评测维度可配置体系 | 关联公共库条目与上传样本 |
| EvalSetComposition | Agent 评测前的评测集组合（单个/维度/多来源合并） | 关联 EvaluationRun 与审计记录 |
...
- 评测集组合选择与临时标准化评测集的创建；
- 公共评测集库条目的新增、更新与停用；
- P0 判定、P0 异常动作（删除/停用、移出分母、未覆盖发布）的用户确认记录；
- 质量评估评分、提示性门禁确认与语义评分校准集版本。
...
| AC-11 | 失败用例可记录为 `E2E` 端到端失败或 `D9` 运行异常，并保留对应运行信息 |
| AC-12 | 可基于同一冻结评测集比较两个待测智能体版本，并展示总体及分组分数变化 |
...
| AC-24 | 不适用：平台不建设评测后回流；智能体失败用于诊断和优化，评测集内容问题走质量复核与版本编辑 |
...
| AC-29 | 可直接上传已有 QA 评测集（单轮 `q/a/evidence/dimension`；可选多轮 `session_id + turns[]`），经格式校验、质量评估与治理审核后入库 |
| AC-30 | 公共评测集库由预置条目构成，用户可查看、按维度选择使用，不开放共享入库；条目版本化且新增/更新/停用留痕 |
| AC-31 | Agent 评测前可指定单个评测集、勾选公共库维度或合并多来源形成临时标准化评测集，组合作为运行配置记录并审计 |
| AC-32 | 向量化以 EIU 为核心对象；EIU 覆盖率、85% 门禁与 P0 全覆盖仅适用文档生成评测集，上传评测集与公共库不参与计算 |
| AC-33 | 跨页 EIU 证据可逐条溯源：存储层保留全字段有序证据列表，审查视图默认只展示原文片段与页码·条款，详情可查全字段 |
| AC-34 | EIU 上下文补全的每个限定语均有对应证据；补全无证据判 revise/block |
| AC-35 | 平台自动判断相对文档目的的重要性并保留理由；前端仅展示正常知识点、待复核和已排除，不展示 P0/P1/P2；测试全量测仍须覆盖全部绿色 P0，且不得豁免 S0 强制规则 |
| AC-36 | EIU/QA/上传评测集三套量化质量评估可在平台展示；评分低于阈值时提示用户确认，S0 硬门禁不可绕过 |
| AC-37 | 无证据样本带降级标注，不参与证据回溯率统计 |
| AC-38 | 多轮评测集可运行并保存完整对话过程，memory/coherence 按声明的关键轮与依赖轮规则评分 |
...
- 定义第一版目标题型分布；
- 定义待测系统接口和基线；
- 一次性确认 P0/P1/P2 规则包（P0 判定以确定性规则为主，见 8.3）；
- 确认上传评测集模板、公共评测集库首批 QA 数据集与评测维度体系（维度暂不固定，见 8.22）；
- 确认语义评分阈值与固定校准集、质量评估提示性门禁阈值。
...
- 评测运行、基础指标、`E2E`/`D9` 黑盒失败记录；
- 整集冻结版本管理与跨智能体版本复测；
- **业务需求书→测试功能点 EIU 提取**（轻量化模式，不生成标准答案）；
- **智能问答交互界面占位**（对话入口、基础消息渲染，不做完整对话交互）。
- **直接上传评测集（单轮模板）与格式校验**；
- **公共评测集库（预置条目）与评测集组合选择**（指定单个评测集/勾选公共库维度/多来源合并）；
- **EIU 质量评估、QA 质量评估与上传评测集质量评估指标展示**（参考评分 + 提示性门禁）；
- **EIU 自动质量门禁、文档质量反馈与按用途选题策略**。
...
- **评测运行结果与跨智能体版本分数比较视图深化**；
- **多轮评测集（memory/coherence）完整实现与完整对话过程保存**；
- **跨页证据审查视图（L2 角色连线、L3 全字段详情）深化**；
- **评测维度体系按首批公共数据集固化**；
- 财务表格、指标公式和单位计算；
- 政策变更、冲突和例外关系；
- 查询分解与迭代检索；
...
16. 直接上传评测集的实际文件格式（JSON/JSONL/Excel/CSV）与字段校验规则，以及多轮测试是否纳入 MVP；
17. 公共评测集库首批 QA 数据集与评测维度体系（维度暂不固定，待首批数据确定后固化）；
18. 语义相似度评分阈值与固定校准集的构建方案（上传评测集长答案评分用）；
19. 三套量化质量评估的提示性门禁阈值与平台界面展示范围；
20. 多轮测试的 `memory`/`coherence` 评分规则与人工抽查比例。
