...
- 运行记录必须保存完整对话过程，用于归因（FR-DIAG-001）。
...
多轮评测（8.22 FR-DS-SRC-002）运行时，适配器应支持按 `session_id` 传入完整对话过程，并返回每轮输出与最终答案；运行记录必须保存完整对话过程，用于归因（FR-DIAG-001）。
...
平台基于失败归因输出可执行诊断建议，供待测智能体的维护方自行决定是否调优；平台不实施智能体调优，也不根据评测结果修改评测集。
...
| 摘要丢事实 | 检查待测智能体的知识表示和索引版本；评测集不因运行结果自动修改 |
| 答案幻觉 | 强制引用、答案要点约束、无证据拒答、忠实性校验 |
| 原始资料无答案 | 将题目标记为不可回答、请求补充资料或移出数据集 |
| Gold 错误/歧义 | 仅在进入 m08 前由人工质量复核确认后，按 m04/m05 修订并冻结新版本；已开始的运行不改写 |
