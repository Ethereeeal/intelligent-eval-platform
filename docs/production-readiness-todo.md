...
1. **运行时联调测试**（需 MySQL / 后端服务）：正常上传、重复上传、同名覆盖确认（token）、重传闭环各 job 阶段与失败回滚；前端确认面板全流程；
2. **认证与访问控制**：当前无鉴权且 CORS 全开（`allow_origins=["*"]` + `allow_credentials=True`）；生产必须加访问令牌 / 反向代理鉴权，CORS 收紧到固定前端源；文档 / 文件夹删除类操作补审计；
3. **压缩炸弹防护**：PDF / XLSX / DOCX 解析前限制页数 / 行数 / 单元格数 / 解压后大小（当前只限制上传字节数）；
4. **`delete_folder` 的 LIKE 通配符误删**：文件夹名含 `%` / `_` 时 `folder_path.like()` 会误匹配其他目录，需转义或改前缀判断；
5. **上传并发唯一约束兜底**：同哈希并发上传可能撞 `file_hash` UNIQUE 抛未捕获异常，需捕获并返回明确错误。

### P1（生产化阶段）

6. **同文档并发重传加锁**：按 document 互斥，避免产生多个新文档或重复删除；
7. **数据库索引与迁移规范**：`file_name` / `folder_path` 等加索引；把启动时 `ALTER` 迁移改为版本化迁移流程；
8. **MinIO 接入**（满足触发条件后实施）：`STORAGE_BACKEND=local|minio` 开关（默认 local、不可用时降级）；bucket 初始化（`bucket_exists` + `make_bucket`）；解析临时目录方案；`minio_path` 语义改为对象 key；密钥只走 `.env` / compose；
9. **评分策略落地**：短答案规范化精确匹配、长答案语义评分 + 固定校准集验证阈值；LLM 裁判防位置 / 长度偏差并保留人工抽查（BRD §8.22 FR-DS-SRC-003）；
10. **日志 / 监控 / 审计**：job 卡死检测（超时置 failed）、token 计数与告警、上传 / 覆盖 / 删除操作审计；
11. **依赖安全**：锁版本并做漏洞扫描（PyMuPDF / openpyxl / python-docx / minio 等）；Docker 镜像最小化与安全基线；
12. **多实例化准备**：`confirm_token` 内存态改 Redis；EIU FAISS 索引多实例共享 / 重建策略；后台线程任务改分布式任务队列。

### P2（业务口径确认，对应 BRD §18）

13. 上传评测集实际格式与字段校验、多轮场景是否纳入 MVP；
14. 公共评测集库首批 QA 数据集与评测维度体系（维度暂不固定）；
15. 语义评分阈值与固定校准集构建方案；
16. 三套量化质量评估（EIU / QA / 上传评测集）提示性门禁阈值与界面展示范围；
17. 多轮 `memory` / `coherence` 评分规则与人工抽查比例；
18. 其余 BRD §18 确认项（数据规模、扫描件比例、外部模型允许、部署形态等）。
