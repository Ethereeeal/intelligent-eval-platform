...
- **评测集合并页重组与展示库重构（2026-08-24，front-func 分支，待提交）**：
  ...
  - **静态校验**：改动的前端 JS（`08-evalset / 02-nav / 04-upload / 07-init`）均通过 `node --check`；
  - 当前通过 `docker cp` 临时同步至 `evalforge-studio` 容器（8080），**未写进镜像**，待 git 提交并 `docker compose build studio` 固化。
