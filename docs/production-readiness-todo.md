...
- **评测集展示目录化与问题分类收敛（2026-08-24）**：移除展示页横向视图栏，改由左侧「评测集展示」展开选择生成库、上传库、公共库、评测集库及输入文档库；四类库均以库文档目录为入口。生成库目录树从「泛化问题 / 基础问题」双根合并为单一「库文档」根，取消类型列、筛选与两类系统根的更多菜单；用户目录和库文档的导出、移动、重命名、删除等更多操作保持可用，后端历史类型字段继续兼容。
