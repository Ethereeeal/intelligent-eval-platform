...
- 文件大小限制（≤ 50MB，上传流式限长读取，超限返回 413，不整读入内存）
- 解析资源上限（P0 压缩炸弹防护）：PDF ≤ 500 页、表格 ≤ 10 万行 / 50 万单元格、DOCX ≤ 20 万元素，超限拒绝解析（见 config.py）
...
**触发方式：**
- 由调用方明确指定已有文档并调用 `POST /api/documents/{document_id}/reupload`；显式重传仍需一次性确认令牌以防误覆盖（不绑定内容哈希）；
