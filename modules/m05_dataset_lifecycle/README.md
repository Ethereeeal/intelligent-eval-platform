...
### 8.14 增量更新与问题生成（BRD 需求；Demo 不做，采用覆盖式全量重算替代）

| 需求编号 | 需求 | Demo 处置 |
|---|---|---|
| FR-DS-INC-001 | 增量 EIU 失效/替代回写：新增文档、或同文档 content_hash 变化（更新）使旧 EIU 被替代/冲突/失效时，标记 superseded/conflicted/deprecated | Demo 不做增量；重传采用覆盖式整体作废+全量重算（见 §3.3） |
| FR-DS-INC-002 | 增量问题生成：仅对新增/变化 EIU 补题，未变化 EIU 已审核题目直接复用 | Demo 不做；重传后整体重新生成题面 |
...
- [ ] （可选）编辑批次与版本草案
- [ ] （Demo 不做）文档重传采用覆盖式全量重算（见 §3.3）；不做增量更新与 EIU 回写（FR-DS-INC 延后）
