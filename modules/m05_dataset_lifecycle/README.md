...
编辑后行为：
  - 草案中的修改直接保存，无需每次生成新版本
  - case.review_status 回退到 candidate；完整的草案重质检/覆盖率重算编排待后续版本实现
  - 冻结或正式发布版本不可直接编辑，必须新建草案；版本冻结（freeze）为独立的显式操作，编辑不自动触发
```
...
- [x] 冻结/发布版本只读保护（PUT/DELETE 返回 409）；草案编辑 API 保留
...
- [ ] 冻结/发布版本的新草案创建、编辑批次与审计（FR-DS-EDIT-003/004）
