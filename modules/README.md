...
- 直接上传评测集（单轮模板为默认，多轮 `turns[]` 字段可保留）+ 质量评估；公共评测集库（预置条目）+ 维度选择；评测集组合选择（m05 §8.22）
...
- 多轮评测集完整逐轮评分（memory/coherence，阶段 2）；第一版只做固定历史上下文 + 最终轮标准答案的精确/语义初筛，失败样本再结合前置对话分析；语义评分固定校准集（待确认）
- 评测后数据回流（06，后续版本）
- 智能问答交互（07，Demo 不做）
