...
> 质量校验需**同时覆盖"问题侧"与"答案侧"**：答案侧检查答案是否忠实于原文（#1–#4），问题侧检查生成的问题是否扎根于文档内容、与原文主题相关（#5）。只校验答案会漏掉"问题本身与文档无关 / 被臆造"的情况——这类问题可能碰巧能被某段原文答上而通过答案侧检查。
...
对每个 eval_case：
  ...
  5. 失败分流（_check_and_handle）：
     ...
     - 有 hard 失败（answerability / faithfulness / question_relevance）→ 自动回 m03 换角度重生成（最多 2 次）：
...
candidate / needs_review ──→ quality_verified
          ...
          │     hard 失败且自动重生成仍失败 / EIU 缺失：题目生成有问题，交人工
          │
          └──→ retired（重生成成功后，原失败 case 退役保留审计）
```

`published` 不在 m04 批量质检范围内；发布与用户确认由后续数据集生命周期流程负责。

Demo 阶段：m04 的终点是 5 项全部通过后的 `quality_verified`；用户确认和 `published` 由 m05 数据集生命周期流程负责。

- **review_tag = `answer_coverage`**：`uniqueness` / `evidence_sufficiency` 失败（soft），因质检可见上下文 ≈ 生成时上下文，无法获得新信息，自动重生成无效，故留人工复核答案/证据覆盖度。
- **review_tag = `generation_issue`**：`answerability` / `faithfulness` / `question_relevance` 失败（hard，不可回答/幻觉/偏题），自动回 m03 换角度重生成最多 2 次仍失败，或 case 无 EIU 无法重生成，说明题目本身生成有问题，交人工。
- **retired**：hard 失败重生成成功后，原失败 case 标记 retired 保留审计痕迹，新 case 正式替代。
- **批量范围**：`POST /api/quality-check` 只检查 `candidate`、`needs_review`；`quality_verified` 需调用单题 `retry-check` 才会复检。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/quality-check?document_id=` | 触发待质检样本校验（仅 `candidate` / `needs_review`，可按文档） |
...
- [x] 质量校验 Prompt（`services/prompts.py`）
- [x] quality_checker 服务：逐题调用 LLM 做 5 项检查（含问题相关性）
- [ ] （可选/后续）补充质量维度 补充-01~08（表意清晰度/问答匹配度/要点完备性/术语一致/内部无矛盾/题型分布/答案精简/无主观，见 §2.5）
- [x] `quality_check_result` 表 + API
- [x] 校验结果汇总 API（按检查类型统计）
- [x] case.review_status 状态更新（passed → quality_verified；soft 失败 → needs_review + answer_coverage；hard 失败自动重生成，成功则原 case retired + 新 case quality_verified，失败则 needs_review + generation_issue）
