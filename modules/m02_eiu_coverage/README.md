...
EIU (Evaluable Information Unit) = 一条能够被原文**独立证明或否定**的最小业务陈述。EIU 门禁只检查忠实性与可追溯性、上下文完整性和原子性；能否形成好题由后续 QA 生成与质检判断。
...
**重要性与自动门禁（BRD V1.4）**：规则先产出类型、结构字段和重要性信号；在同一文档全部候选齐备后，LLM 结合文档目的、目录和章节结构批量判定内部 P0/P1/P2，离线时保守降级而不把规则类型误判为 P0。三项质量门禁通过即为绿色并可自动出题；未通过的 P0 为黄色“待复核”，未通过的 P1/P2 为红色“已排除”。
...
- **EIU 质量评估**：三项自动门禁（忠实性与可追溯性、上下文完整性、原子性）是出题前硬门禁；不在 EIU 阶段检查“可生成性/可测试性”。文档质量仅反馈问题和定位，不打总分、不要求逐条人工确认。
- **性能与失败保护**：规则候选先按同文档全文分批（每批最多 20 条、默认并发 3）判断相对重要性；三项门禁已通过的候选直接进入收口，P2 首次不合格直接归档，只有不合格的 P0/P1 调用 LLM 补证。LLM 默认单次超时 30 秒、瞬时故障最多重试一次，连续 3 次审查失败后本任务熔断；失败候选绝不降级转绿。抽取进度按 Block 更新，statement embedding 在去重与落库间复用。
- **人工例外**：黄色知识点编辑后必须重跑三项门禁和重要性判定；可人工指定纳入作单题实验，但不覆盖系统结论，也不混入正式自动题库或覆盖统计。红色项只读。
