...
调用排障：WARNING 日志包含调用编号、尝试次数、单次/累计耗时、异常类型、HTTP 状态、输入字符数和是否重试；INFO 成功日志包含响应结束原因、输出字符数与 token 用量（需启用 `LOG_LEVEL=INFO`）。JSON 解析失败单独记录。日志不输出模型输入/响应正文或原始异常消息；这些调用明细位于后端应用日志，尚未接入前端处理轨迹。超时和重试仍由 `EIU_LLM_TIMEOUT_SECONDS` / `EIU_LLM_MAX_ATTEMPTS` 控制。

> 覆盖 BRD：8.3 语义理解与知识编译 / 8.5 覆盖规划 / §8.24（V1.4）
...
| FR-SEM-003 | EIU 抽取：将段落拆为可追溯业务知识单元，逐条结构化记录；抽取完成后按同文档整体上下文判定相对重要性，并保留证据与理由（BRD V1.4） |
...
EIU (Evaluable Information Unit) = 一条能够被原文**独立证明或否定**的最小业务陈述。EIU 硬门禁只检查忠实性与可追溯性、上下文完整性；原子性作为出题提示，由后续 QA 生成与质检处理。
...
拆分结果：
  EIU-1 [rule/threshold] 小微企业申请流动资金贷款时，资产负债率原则上不得超过70%
  EIU-2 [exception/threshold] 由政策性担保机构提供全额担保时，资产负债率上限可放宽至75%
  EIU-3 [date] 该规定自2026年8月1日起施行

抽取完成后才依据文档目的、章节结构和全量候选，为上述 EIU 判定内部 P0/P1/P2；规则类型或“不得/必须”等单条词汇不直接决定优先级，前端也不展示该内部字段。
```
...
**重要性与自动门禁（BRD V1.4）**：规则先产出类型、结构字段和重要性信号；在同一文档全部候选齐备后，LLM 结合文档目的、目录和章节结构批量判定内部 P0/P1/P2，离线时保守降级而不把规则类型误判为 P0。忠实性与上下文完整性通过即为绿色并可自动出题；原子性只生成 QA 出题提示；未通过硬门禁的 P0 为黄色“待复核”，未通过的 P1/P2 为红色“已排除”。
...
- LLM 响应中的 `block_id` 视为不可信输入：缺失或非法时回退到当前 Block；单个 Block 的模型、上下文或规范化异常会保留证据并进入后续文档级门禁，不会中断整篇文档。
...
**上下文补全与证据列表化（BRD V1.4，已按同文档范围实现基础能力）：**

- **上下文补全/物化**：真值条件（主体/条件/范围/期间/币种/单位/例外）补全进 EIU 陈述，补全不新增 EIU、不重复计覆盖率；抽取前置"上下文清单"（文档级全局条款），逐块做"上下文充分性检查"，补全必可溯源（每个限定语必须有对应证据块，否则判 `revise`/`block`）。
- **证据列表化**：EIU 证据从"`evidence_blocks` 数组"升级为**有序证据列表**（每条含 FR-PARSE-003 全字段定位 + 角色 `direct`/`qualifier`/`reference`/`conflict` + 版本），并支持 `statement_provenance`（陈述片段→证据条目映射）；审查视图默认只展示原文片段 + 页码·条款 + 角色中文名，全字段折叠详情。
- **EIU 质量评估**：忠实性与可追溯性、上下文完整性是出题前硬门禁；原子性只作为 QA 多角度出题提示，不单独阻断。不在 EIU 阶段检查“可生成性/可测试性”。文档质量仅反馈问题和定位，不打总分、不要求逐条人工确认。
- **性能与失败保护**：规则候选先按同文档全文分批（每批最多 20 条、默认并发 3）判断相对重要性；两项硬门禁已通过的候选直接进入收口，原子性警告转为 QA 多角度出题提示，P2 首次不合格直接归档，只有硬门禁不合格的 P0/P1 调用 LLM 补证。LLM 默认单次超时 30 秒、瞬时故障最多重试一次，连续 3 次审查失败后本任务熔断；模型不可用时，仅对核心业务章节中的明确边界候选保守回退 P0，避免重要内容直接归档；失败候选绝不降级转绿。抽取进度按 Block 更新，statement embedding 在去重与落库间复用。
- **人工例外**：黄色知识点编辑后必须重跑两项硬门禁和重要性判定；可人工指定纳入作单题实验，但不覆盖系统结论，也不混入正式自动题库或覆盖统计。红色项只读。
...
> **适用范围（BRD V1.3 §8.22）**：EIU 覆盖率、85% 门禁与 P0 全覆盖**仅适用于文档生成评测集**；直接上传评测集与公共评测集库只包含 QA 对、不产生 EIU，不参与覆盖率计算，只做质量评估与组合选择。
