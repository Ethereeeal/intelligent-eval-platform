...
**上下文补全/物化与证据列表化（BRD V1.3，Demo 不实现）：**
...
- **EIU 质量评估（FR-QA-005）**：四项必查（完整性/独立性/重复率/可生成性）+ 参考指标（信息密度、覆盖范围），作为参考评分体系；质量评分低于阈值时提示用户确认（提示性门禁），S0 硬门禁不可豁免。
- **P0 确认流程**：抽取后展示 P0 清单视图，默认批量通过、只处理异常项；用户确认不得豁免 P0 覆盖率 100% 与 S0 规则；P0 降级走规则变更流程。
