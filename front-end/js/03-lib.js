  ...
  function docContentHTML(docId) {
    ...
    return `<div class="lib-head">
      ...
      <div class="card card-pad">
        ...
        <div data-processing-trace="${escapeHTML(docId)}" class="document-quality" aria-live="polite" hidden></div>
        ${kpTableHTML(d.kp || [])}
      </div>`;
  }
  ...
  function processingTraceSummary(item) {
    ...
    if (item.event === "final_disposition") {
      const state = { yellow: "需要复核", red: "未采用" }[detail.auto_disposition] || "需要关注";
      const reason = detail.exclusion_reason || detail.route_reasons?.[0] || "质量检查未通过";
      return `${state}：${reason}`;
    }
    if (item.event === "failed") return `处理失败：${detail.message || detail.error_type || "未知错误"}`;
    return `需要关注：${detail.reason || detail.message || detail.route_reasons?.[0] || "处理结果存在异常"}`;
  }

  function processingProblemGroups(trace) {
    const stageLabels = {
      upload: "文件接收",
      parse: "文档解析",
      chunk: "内容分段",
    ...
    (trace || [])
      ...
      .forEach(item => {
        const stage = stageLabels[item.stage] || "文档处理";
        const summary = processingTraceSummary(item);
        const key = `${stage}\n${summary}`;
        const group = groups.get(key) || { stage, summary, count: 0 };
        group.count += 1;
  ...
  async function loadProcessingTrace(docId) {
    ...
    try {
      ...
      if (!current()) return;
      const problems = processingProblemGroups(trace);
      const problemCount = problems.reduce((total, item) => total + item.count, 0);
      if (!problemCount) {
        target.hidden = true;
      ...
      target.hidden = false;
      target.innerHTML = `<details><summary>处理问题 · ${problemCount} 项</summary><p class="muted">仅展示未进入可用知识点或需要人工关注的内容；相同原因已合并。</p>${problems.map(item => `<div class="document-quality-finding"><b>${escapeHTML(item.stage)} · ${escapeHTML(item.summary)}</b>${item.count > 1 ? `<p class="muted">涉及 ${item.count} 项</p>` : ""}</div>`).join("")}</details>`;
    } catch (error) {
      if (!current()) return;
      target.hidden = true;
      target.replaceChildren();
