...
(() => {
  ...
  const evaluationConfigController = window.EvaluationConfigUI.createController({ state });
  const { taskProfileLabels, methodOptions, nodeOptions } = evaluationConfigController;
  const bindEvaluationControls = () => evaluationConfigController.bind();
  const readEvaluationControls = () => evaluationConfigController.read();
