...
复用已有 corpus（含 M01 文档与 M02 EIU），跑：
- M02：确认 EIU 数据在
...
CORPUS_ID = 1  # 复用已有 corpus（含 存单质押 docx + 109 EIU）
...
dry = client.post(
    f"/api/corpus/{CORPUS_ID}/cases/generate",
...
cases = client.get(f"/api/corpus/{CORPUS_ID}/cases").json()
check("M03 已生成题目可查", len(cases) >= 3, f"共 {len(cases)} 题")

print()
print("④ M04：质量门禁（对已生成题目跑 5 项检查）")
t0 = time.time()
qr = client.post(f"/api/corpus/{CORPUS_ID}/quality-check")
...
results = client.get(f"/api/corpus/{CORPUS_ID}/quality-check/results")
